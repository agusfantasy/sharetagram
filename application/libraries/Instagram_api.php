...
class Instagram_api {
    ...
    protected $api_urls = array(
		...
		'user_followed_by'			=> 'https://api.instagram.com/v1/users/%s/followed-by?access_token=%s&cursor=%s', 
		...
		//'tags_recent'				=> 'https://api.instagram.com/v1/tags/%s/media/recent?access_token=%s&count=150',
		...
		'user_media_liked'			=> 'https://api.instagram.com/v1/users/self/media/liked?access_token=%s&max_like_id=%s'
    );
    ...
    function instagramLogin() {
    	...
    	return 'https://api.instagram.com/oauth/authorize/?client_id=' . $this->codeigniter_instance->config->item('instagram_client_id') . '&redirect_uri=' . $this->codeigniter_instance->config->item('instagram_callback_url') . '&response_type=code&scope=comments+relationships+likes';
     ...
     */
    function __apiCall($url, $post_parameters = FALSE, $delete = FALSE) {
     ...
     */
    function userFollows($user_id, $cursor=null) {
    	
    	$user_follows_request_url = sprintf($this->api_urls['user_follows'], $user_id, $this->access_token, $cursor);
     ...
     */
    function userFollowedBy($user_id, $cursor=null) {
    	
    	$user_followed_by_request_url = sprintf($this->api_urls['user_followed_by'], $user_id, $this->access_token, $cursor);
    ...
    function removeLike($media_id) {
		...
		return $this->__apiCall($remove_like_request_url,FALSE,TRUE);
	...
	/*function tagsRecent($tag) 
	{
		$tags_recent_request_url = sprintf($this->api_urls['tags_recent'], $tag,$this->access_token);    	
    	return $this->__apiCall($tags_recent_request_url);    
    }*/
	...
	*/
	function userMediaLiked($max_like_id=null) {
    
    	$user_media_liked_url = sprintf($this->api_urls['user_media_liked'], $this->access_token, $max_like_id);
    	
    	return $this->__apiCall($user_media_liked_url);
